...
const markup = (data) => {
  ...
  const style = css(
    `${fs.readFileSync(
      path.join(__dirname, '../_includes/assets/styles/main.css')
    )}`
  );
  const langSwitch = (lang) => (lang === 'ja' ? `lang=${lang}` : '');
  const dateSwitch = (date) => (date === true ? `(${date})` : '');
        ...
        <article class="c-preview c-post h-entry" ${langSwitch(data.lang)}>
          ...
          <div class="c-preview__inner-wrapper">
            <figure class="o-blockquote">
              ...
              <figcaption class="o-blockquote__attribution">
                — ${tags(data.tags)} ${data.cite} ${dateSwitch(data.when)}
              </figcaption>
            </figure>
          </div>
        </article>
...
module.exports = class {
  ...
  async render(data) {
    ...
    const hasCache = () =>
      cacheList.filter((item) => item.includes(fileSlug))[0];
